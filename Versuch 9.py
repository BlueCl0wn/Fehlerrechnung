...
graph(I_2, ((B_M(I_2), r"B_M(I_2) in $T$"), (B_1(U_H_2),r"B(U_H_2) in $T$")), title="2a.", trendlinie=True, xlabel="$I$ in $A$", ylabel=r"$B(I)$ in $T$")
...
graph(d_M, ((B_x_1, r"$B_t (x)$"), (B_x_2, r"$B(x)$")), title=r"2b.", xlabel=r"$d_M$ in $m$", ylabel=r"$B(x)$ in $T$")
...
print(f"B_21 (Umlaufsinn entgegengesetz)= {[round(i, 6) for i in B_21]}")
...
I_30 = [0.75, 1.5, 2.25, 3]  # Stromstärke I sind bei Stromrichtung gleich und entgegengesetzt die selben Werte
...
graph(I_30, ((F_30, r"Stromrichtung gleich"), (F_31, r"Stromrichtung entgegengesetzt")), title="3c.", xlabel=r"$z$ in $m$", ylabel=r"magnetische Kraft $F$ in $N$", trendlinie=True)
